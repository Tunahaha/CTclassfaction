...
# 3.進行矩陣乘法(用@)
# 4.加上原點偏移輛
IrcTuple = collections.namedtuple('IrcTuple', ['index', 'row', 'col'])#(設置tuple的名稱，設置tuple中元素的名稱)
XyzTuple = collections.namedtuple('XyzTuple', ['x', 'y', 'z'])

def irc2xyz(coord_irc, origin_xyz, vxSize_xyz, direction_a):#IRC轉換成XYZ的韓式
    cri_a = np.array(coord_irc)[::-1] #第一步
    origin_a = np.array(origin_xyz)#篇一輛
    vxSize_a = np.array(vxSize_xyz)
    coords_xyz = (direction_a @ (cri_a * vxSize_a)) + origin_a#第三、第二、第四步
    return XyzTuple(*coords_xyz)

def xyz2irc(coord_xyz, origin_xyz, vxSize_xyz, direction_a):#XYZ轉成IRC
